...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| `hello` | 2,281 / 471 | 1,485 / 300 | 238 | 214 | 24 | 1,247 | 62 |
| `loadable_all` | 2,420 / 497 | 1,608 / 342 | 271 | 243 | 28 | 1,337 | 71 |
| `loadable_apps` | 2,424 / 497 | 1,612 / 342 | 271 | 242 | 29 | 1,341 | 71 |
| `xip_all` | 2,405 / 492 | 1,593 / 337 | 267 | 229 | 38 | 1,326 | 70 |
...
| `CONFIG_ENABLE_PS` | 2215: `n` | 450: `y` |
| `CONFIG_EXAMPLES_TESTCASE` | 2136: `n` | 366: `y` |
| `CONFIG_FLASH_PART_NAME` | 1055: `"bl1,sys_its,sys_ps,ss,kernel,kernel,userfs,reserved,easy_flash,bootparam,"` | 275: `"kernel,userfs,"` |
| `CONFIG_FLASH_PART_SIZE` | 1053: `"92,8,8,404,1856,1856,2048,10080,24,8,"` | 273: `"640,3456,"` |
| `CONFIG_FLASH_PART_TYPE` | 1054: `"none,none,none,none,kernel,kernel,smartfs,none,none,bootparam,"` | 274: `"kernel,smartfs,"` |
| `CONFIG_FLASH_SIZE` | 1041: `16777216` | 270: `4194304` |
| `CONFIG_FLASH_START_ADDR` | 1040: `0x0` | 269: `0x80c00000` |
| `CONFIG_FS_AUTOMOUNT_PROCFS` | 1711: `n` | 465: `y` |
| `CONFIG_INTELHEX_BINARY` | 33: `y` | 25: `n` |
| `CONFIG_NET` | 1398: `y` | 245: `n` |
| `CONFIG_RAMMTD` | 1751: `n` | 286: `y` |
...
| `CONFIG_SCHED_CHILD_STATUS` | 1126: `n` | 459: `y` |
| `CONFIG_STACK_OVERFLOW_PROTECTION_DISABLE` | 113: `y` | 63: `n` |
| `CONFIG_UART0_TXBUFSIZE` | 1315: `1024` | 236: `4096` |
...
`loadable_all`의 QEMU-only 71개는 QEMU board/chip, RAMMTD erase, tmpfs,
worker priority, semaphore holder, ITC/kernel testcase, UART buffer를
포함한다. 특히 `CONFIG_RAM_KREGIONx_HEAP_INDEX`가 BK의 `0,1,2`와 QEMU의
`0,2,1`로 다르므로, 세 번째 heap이 실행 이미지 heap인지 확인할 때 index와
주소/크기를 함께 읽어야 한다.

### `loadable_apps` — 29개

`loadable_apps`는 `loadable_all`의 28개 차이에 다음 한 항목이 추가된다.

```text
CONFIG_APP1_BIN_DYN_RAMSIZE: BK=2048000, QEMU=2097152
```

나머지 28개는 `loadable_all`과 동일하다. 즉 이 recipe의 핵심 추가
차이는 app1 동적 RAM budget이며, QEMU package loader를 검증할 때
`app1`의 loadable image 크기와 이 RAM budget을 함께 확인해야 한다.
...
`xip_all`의 QEMU-only 70개에는 ELF loadable buffer/dual-slot,
...
있다. BK는 16 MiB 실제 flash partition, 네트워크, Intel HEX와 vendor
PSRAM/SoC 설정을 가진다.
...
BK-only 설정 1,326~1,341개는 vendor HAL, 실제 flash/PSRAM, TrustZone,
Wi-Fi/BLE, GPIO/SPI/I2C/PWM 같은 hardware 기능이다. QEMU-only 설정
62~71개는 QEMU board glue, RAMMTD, tmpfs, test provider, ITC/kernel
testcase와 loader buffer다. 이 설정을 숫자만 맞추는 것은 안전하지 않으며,
메모리 주소·linker script·board source·package layout을 함께 변경해야
한다.
...
2026-07-25에 임시 checkout에서 각 recipe를 `./os/dbuild.sh 5`로 clean
build한 뒤 configure/build와 QEMU runner를 순서대로 실행했다.

| recipe | clean build | QEMU runner | 결과 |
| --- | --- | --- | --- |
| `hello` | kernel `393,032 / 655,360` PASS | `PASS 459, FAIL 0` | PASS |
| `loadable_all` | partition check PASS | `PASS 447, FAIL 0` | PASS |
| `loadable_apps` | partition check PASS | `PASS 447, FAIL 0` | PASS |
| `xip_all` | partition check PASS | `PASS 447, FAIL 0` | PASS |
...
| oversized package | `loadable_all` app1 header size를 1 MiB 초과로 변조 | `Invalid Header data`, success marker 없음 | expected-rejection |
| alternate slot | 손상된 app1로 bootparam recovery 유도 | set B 선택 후 재부팅, `PASS 447, FAIL 0` | PASS, active slot 1 |
...
```bash
./os/dbuild.sh 5
./os/tools/configure.sh qemu-armv8m/hello
./os/dbuild.sh 1
