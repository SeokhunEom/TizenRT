...
while True:
 ...
 time.sleep(10)
all_results=[r for r in json.loads((base/'qemu-matrix.json').read_text()) if r['status']=='pass']
...
def execute(name,cmd,out,timeout=1200):
 ...
 if status=='pass' and ('fatal-' in name or '-reset-' in name or '-expiry-' in name or '-user-expire' in name):
...
for label,profile,config in variants:
 ...
 out=base/('qemu-'+label+'-build');reuse=(out/'result.json').exists();out.mkdir(exist_ok=True)
 ...
 def run(case,rounds=3,ms=None):
  ...
  if any(r['name']==name and r['status']=='pass' for r in all_results):return
 ...
 if label=='hello-off':
  ...
  run('normal');policy='reset' if 'CONFIG_BOARD_ASSERT_AUTORESET=y' in config.read_text() else 'expiry';run(policy,ms=1000);run(policy,ms=2000)
