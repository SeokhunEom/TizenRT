...
flash_size_value = config.get("CONFIG_FLASH_SIZE")
part_sizes_value = config.get("CONFIG_FLASH_PART_SIZE")
part_names_value = config.get("CONFIG_FLASH_PART_NAME")

if not flash_size_value or not part_sizes_value or not part_names_value:
    missing = [
        key
        for key, value in (
            ("CONFIG_FLASH_SIZE", flash_size_value),
            ("CONFIG_FLASH_PART_SIZE", part_sizes_value),
            ("CONFIG_FLASH_PART_NAME", part_names_value),
        )
        if not value
    ]
    raise SystemExit("Missing flash layout config: " + ", ".join(missing))

flash_size = int(flash_size_value, 0)
part_sizes = [item for item in part_sizes_value.split(",") if item]
part_names = [item for item in part_names_value.split(",") if item]
