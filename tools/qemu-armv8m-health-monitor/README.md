...
The fault-message runner and metadata extractor use explicit fatal checks; Python
optimization does not remove ACK reads, breakpoint requests, or verdict checks.
Run `python3 test-fault-message.py`, `python3 -O test-fault-message.py`, and
`python3 -OO test-fault-message.py` from this directory for protocol regressions.
