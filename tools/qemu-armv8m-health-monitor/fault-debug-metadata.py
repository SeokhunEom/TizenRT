...
a = p.parse_args()
expressions = {'message_size': 'sizeof(binmgr_request_t)',
...
text = subprocess.check_output(command, stderr=subprocess.STDOUT).decode()
(a.build / 'fault-debug-metadata.txt').write_text(text)
meta = {key: int(value) for (key, value) in re.findall('HMDBG (\\w+)=(\\d+)', text)}
if set(meta) != set(expressions):
    raise RuntimeError('Incomplete debugger metadata')

def instructions(function):
    section = text.split('Dump of assembler code for function ' + function + ':')[1].split('End of assembler dump.')[0]
    return [(int(addr, 16), op) for (addr, op) in re.findall('^\\s*(0x[0-9a-f]+) <\\+\\d+>:\\s*(.+)$', section, re.M)]

def call(function, target):
    seq = instructions(function)
    hits = [i for (i, (_, op)) in enumerate(seq) if re.search('\\bbl\\s+.*<' + target + '>', op)]
    if len(hits) != 1:
        raise RuntimeError((function, target, hits))
    i = hits[0]
    return (seq, i)
(seq, i) = call('binary_manager_faultmsg_sender', 'mq_send')
meta['send_call'] = seq[i][0]
meta['send_return'] = seq[i + 1][0]
(seq, i) = call('binary_manager', 'mq_receive')
meta['receive_return'] = seq[i + 1][0]
setup = '\n'.join((op for (_, op) in seq[max(0, i - 5):i]))
offset = re.findall('add(?:\\.w)?\\s+r1, sp, #(\\d+)', setup)
if len(offset) != 1:
    raise RuntimeError(setup)
meta['receive_buffer_sp_offset'] = int(offset[0])
(seq, i) = call('binary_manager', 'binary_manager_recovery')
meta['dispatch_call'] = seq[i][0]
config = (a.build / 'effective.config').read_text()
if 'CONFIG_ARM_CMNVECTOR=y' not in config:
    raise RuntimeError('Common ARM exception context required')
meta['context_pc_offset'] = 4 * (11 + (16 if '\nCONFIG_ARCH_FPU=y\n' in config else 0) + (2 if '\nCONFIG_REG_STACK_OVERFLOW_PROTECTION=y\n' in config else 1) + 6)
for key in ['usage_entry', 'mpu_entry', 'hard_entry', 'recovery_entry', 'userfault_entry']:
    meta[key] &= ~1
