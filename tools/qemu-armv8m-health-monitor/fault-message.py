...
class Remote:
    ...
    def packet(self):
        ...
        if not (int(checksum, 16) == sum(raw) & 255):
            raise RuntimeError('Validation failed: int(checksum, 16) == sum(raw) & 255')
    ...
    def send(self, text):
        ...
        if not (self.receive(1) == b'+'):
            raise RuntimeError("Validation failed: self.receive(1) == b'+'")
    ...
    def memory(self, address, size):
        ...
        if not (len(data) == size):
            raise RuntimeError('Validation failed: len(data) == size')
        return data
    ...
    def breakpoint(self, address, enabled=True):
        if not (self.request(('Z' if enabled else 'z') + f'1,{address:x},2') == 'OK'):
            raise RuntimeError("Validation failed: self.request(('Z' if enabled else 'z') + f'1,{address:x},2') == 'OK'")
...
def main():
    ...
    if not (meta['elf_sha256'] == hashlib.sha256((a.build / 'bin/tinyara').read_bytes()).hexdigest()):
        raise RuntimeError("Validation failed: meta['elf_sha256'] == hashlib.sha256((a.build / 'bin/tinyara').read_bytes()).hexdigest()")
    if not (meta['config_sha256'] == hashlib.sha256((a.build / 'effective.config').read_bytes()).hexdigest()):
        raise RuntimeError("Validation failed: meta['config_sha256'] == hashlib.sha256((a.build / 'effective.config').read_bytes()).hexdigest()")
    if not ('CONFIG_BINMGR_RECOVERY=y' in config and 'CONFIG_BINMGR_RELOAD_REBOOT=y' not in config):
        raise RuntimeError("Validation failed: 'CONFIG_BINMGR_RECOVERY=y' in config and 'CONFIG_BINMGR_RELOAD_REBOOT=y' not in config")
    if not ('CONFIG_EXAMPLES_HEALTH_MONITOR_ARMV8M=y' in config):
        raise RuntimeError("Validation failed: 'CONFIG_EXAMPLES_HEALTH_MONITOR_ARMV8M=y' in config")
    if not (a.app == 'app1' or a.profile != 'xip_all'):
        raise RuntimeError("Validation failed: a.app == 'app1' or a.profile != 'xip_all'")
    ...
    with (a.output / 'serial.log').open('wb') as log, (a.output / 'gdb-rsp.log').open('w') as trace:
        ...
        try:
            ...
            while time.monotonic() < deadline:
                ...
                if pc in (meta['usage_entry'], meta['mpu_entry'], meta['hard_entry']):
                    if not (fault is None):
                        raise RuntimeError('Repeated CPU fault before message delivery')
                    expected = meta['usage_entry'] if a.kind == 'udf' else meta['mpu_entry']
                    if not (pc in (expected, meta['hard_entry'])):
                        raise RuntimeError('Wrong CPU exception')
                    actual = task()
                    if not (actual['binidx'] == (1 if a.app == 'app1' else 2)):
                        raise RuntimeError("Validation failed: actual['binidx'] == (1 if a.app == 'app1' else 2)")
                    if not (actual['health_timeout'] == (0 if a.unregistered else 60000)):
                        raise RuntimeError("Validation failed: actual['health_timeout'] == (0 if a.unregistered else 60000)")
                    ...
                    if not (regs[0] == (3 if pc == meta['hard_entry'] else (6 if a.kind == 'udf' else 4))):
                        raise RuntimeError("Validation failed: regs[0] == (3 if pc == meta['hard_entry'] else (6 if a.kind == 'udf' else 4))")
                    if pc == meta['hard_entry']:
                        if not (int(fault['hfsr'], 16) & 0x40000000):
                            raise RuntimeError('Expected forced HardFault escalation')
                    if a.kind == 'udf':
                        if not (int(fault['cfsr'], 16) & 0x10000 and fault['instruction'].startswith('00de')):
                            raise RuntimeError("Validation failed: int(fault['cfsr'], 16) & 0x10000 and fault['instruction'].startswith('00de')")
                    else:
                        if not (int(fault['cfsr'], 16) & 0x82 == 0x82 and fault['mmfar'] == '0x80000000'):
                            raise RuntimeError("Validation failed: int(fault['cfsr'], 16) & 0x82 == 0x82 and fault['mmfar'] == '0x80000000'")
                    ...
                    if not (fault is not None and sent is None):
                        raise RuntimeError('Validation failed: fault is not None and sent is None')
                    ...
                    if not (sent['message']['cmd'] == 10 and sent['message']['binidx'] == fault['task']['binidx']):
                        raise RuntimeError("Validation failed: sent['message']['cmd'] == 10 and sent['message']['binidx'] == fault['task']['binidx']")
                    if not (sent['length'] == meta['message_size'] and sent['priority'] == 100):
                        raise RuntimeError("Validation failed: sent['length'] == meta['message_size'] and sent['priority'] == 100")
                    if not (int(sent['task']['tcb'], 16) == remote.word(meta['sender_tcb'])):
                        raise RuntimeError("Validation failed: int(sent['task']['tcb'], 16) == remote.word(meta['sender_tcb'])")
                    remote.breakpoint(pc, False); active.remove(pc)
                elif pc == meta['receive_return']:
                    msg = message(regs[13] + meta['receive_buffer_sp_offset'])
                    if msg['cmd'] == 10:
                        if not (sent is not None and regs[0] == meta['message_size']):
                            raise RuntimeError("Validation failed: sent is not None and regs[0] == meta['message_size']")
                        if not (msg['raw'] == sent['message']['raw']):
                            raise RuntimeError("Validation failed: msg['raw'] == sent['message']['raw']")
                    ...
                    if not (fault is not None and sent is not None and received is not None):
                        raise RuntimeError('Validation failed: fault is not None and sent is not None and received is not None')
                    if not (regs[0] == fault['task']['binidx']):
                        raise RuntimeError("Validation failed: regs[0] == fault['task']['binidx']")
            ...
            if not (notice and int(notice[1]) == fault['task']['pid'] and int(notice[2]) & 1):
                raise RuntimeError("Validation failed: notice and int(notice[1]) == fault['task']['pid'] and int(notice[2]) & 1")
            if not (int(notice[3]) == (not a.unregistered)):
                raise RuntimeError('Validation failed: int(notice[3]) == (not a.unregistered)')
            expected_type = 'UNDEFINSTR' if a.kind == 'udf' else 'DACCVIOL'
            if fault['irq'] != 3:
                if not ('FAULT TYPE: ' + expected_type in text):
                    raise RuntimeError("Validation failed: 'FAULT TYPE: ' + expected_type in text")
            else:
                if not (re.search(r'CFAULTS:\s*0*' + fault['cfsr'][2:] + r'\b', text)):
                    raise RuntimeError("Validation failed: re.search(r'CFAULTS:\\s*0*' + fault['cfsr'][2:] + r'\\b', text)")
            if not (len(re.findall(r'Assertion failed at file:', text)) == 1):
                raise RuntimeError("Validation failed: len(re.findall(r'Assertion failed at file:', text)) == 1")
            if not (('up_hardfault.c' if fault['irq'] == 3 else ('up_usagefault.c' if a.kind == 'udf' else 'up_memfault.c')) in text):
                raise RuntimeError("Validation failed: ('up_hardfault.c' if fault['irq'] == 3 else ('up_usagefault.c' if a.kind == 'udf' else 'up_memfault.c')) in text")
            if not (re.search(r'Assert location \(PC\) : 0x0*' + fault['fault_pc'][2:], text)):
                raise RuntimeError("Validation failed: re.search(r'Assert location \\(PC\\) : 0x0*' + fault['fault_pc'][2:], text)")
            if not (not re.search(r'health_monitor_timeout:|HEALTH MONITOR TIMEOUT|Try to recover fault|Failed to deactivate binary', text)):
                raise RuntimeError("Validation failed: not re.search(r'health_monitor_timeout:|HEALTH MONITOR TIMEOUT|Try to recover fault|Failed to deactivate binary', text)")
