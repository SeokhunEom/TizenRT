...
TESTCASE_END_RE = re.compile(
    r"End\s+\[PASS\s*:\s*(?P<pass>\d+),\s*FAIL\s*:\s*(?P<fail>\d+)\]",
    re.DOTALL,
)
TESTCASE_SUMMARY_RE = re.compile(
    r"PASS\s*:\s*(?P<pass>\d+)\s*,\s*FAIL\s*:\s*(?P<fail>\d+)",
    re.DOTALL,
)
...
class QemuHarness:
    ...
    @staticmethod
    def _extract_testcase_summary(text: str):
        match = TESTCASE_END_RE.search(text)
        if match:
            return match
        return TESTCASE_SUMMARY_RE.search(text)
    ...
    def run_testcase(self, command: str, timeout: int | None = None) -> None:
        started_at = time.monotonic()
        error: Exception | None = None
        try:
            self._send_command(command)
            effective_timeout = timeout or self.tc_timeout
            deadline = time.monotonic() + effective_timeout
            patterns: list[object] = [TESTCASE_END_RE, self.prompt]
            all_patterns: list[object] = patterns + self.fail_markers + [pexpect.EOF, pexpect.TIMEOUT]
            ...
            while time.monotonic() < deadline:
                ...
                index = self.child.expect(all_patterns, timeout=remaining)

                if index == 0:
                    summary_chunks.append(f"{self.child.before}{self.child.after}")
                    prompt_seen = False
                    match = self._extract_testcase_summary("".join(summary_chunks))
                    break

                if index == 1:
                    summary_chunks.append(self.child.before)
                    prompt_seen = True
                    match = self._extract_testcase_summary("".join(summary_chunks))
                    if match:
                        break
                ...
                marker_index = index - len(patterns)
                if marker_index < len(self.fail_markers):
                    raise HarnessError(f"{command}: detected failure marker '{all_patterns[index]}'")
                if all_patterns[index] is pexpect.EOF:
                    raise HarnessError(f"{command}: QEMU exited unexpectedly")
...
def run_platform_steps(harness: QemuHarness, platform: PlatformSpec) -> None:
    for step in platform.steps:
        if isinstance(step, PromptCommandStep):
            ...
            harness.run_testcase(step.command, timeout=step.timeout)
        else:
