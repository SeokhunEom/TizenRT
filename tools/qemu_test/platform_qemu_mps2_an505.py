...
def _resolve_kernel_path(topdir: Path) -> Path:
    ...
    raise RuntimeError(
        "No staged qemu-mps2-an505 runtime image found under "
        f"{image_dir}. Run build/download first and ensure download completed successfully."
    )
...
def build_platform(topdir: Path, config: dict[str, str]) -> PlatformSpec:
    ...
    testcase_timeout = (
        180 if _config_enabled(config, "CONFIG_APP_BINARY_SEPARATION") else None
    )
    ...
    steps = [
        ...
        TestcaseStep("drivers_tc", timeout=testcase_timeout),
        TestcaseStep("filesystem_tc", timeout=testcase_timeout),
        TestcaseStep("kernel_tc", timeout=testcase_timeout),
    ]
