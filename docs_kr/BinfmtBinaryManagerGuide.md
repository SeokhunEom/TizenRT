...
- [Binfmt 흐름](#binfmt-flow)
- [바이너리 관리자 부팅 흐름](#binary-manager-boot-flow)
- [로드 및 실행 흐름](#load-and-execute-flow)
- [상태 머신](#state-machine)
- [Bootparam 및 A/B 파티션](#bootparam-and-ab-partitions)
- [업데이트 흐름](#update-flow)
- [복구 흐름](#recovery-flow)
- [공통 바이너리 처리](#common-binary-handling)
- [디버깅 및 수정 포인트](#debugging-and-modification-points)
...
| 영역 | 파일 | 역할 |
| --- | --- | --- |
| binfmt 공개 API | `os/include/tinyara/binfmt/binfmt.h` | `struct binary_s`, `struct binfmt_s`, `BIN_TEXT`/`BIN_HEAP` 섹션 열거형 |
...
| 바이너리 관리자 공개 API | `os/include/tinyara/binary_manager.h` | 요청/result 열거형, 바이너리 헤더, 공개 등록 API |
| 관리자 내부 | `os/kernel/binary_manager/binary_manager_internal.h` | 스레드 우선 순위, 바이너리 테이블, bootparam 구조, 매크로 접근자 |
| manager thread | `os/kernel/binary_manager/binary_manager.c` | message queue loop와 request dispatch |
| 파티션/state 데이터 | `os/kernel/binary_manager/binary_manager_data.c` | 커널/user 바이너리 테이블, 파티션 등록, 버전 스캔 |
| 로더 정책 | `os/kernel/binary_manager/binary_manager_load.c` | 로더 스레드, 모두/update/reload 로드, 바이너리 | 종료
| bootparam | `os/kernel/binary_manager/binary_manager_bootparam.c` | bootparam 스캔, 검증, 쓰기, 정렬 설정/recovery |
| header/CRC verify | `os/kernel/binary_manager/binary_manager_verify.c` | binary header validation과 CRC check |
| 응답 경로 | `os/kernel/binary_manager/binary_manager_response.c` | 요청자 응답 메시지 큐 전송 |
| 콜백 경로 | `os/kernel/binary_manager/binary_manager_callback.c` | 앱 상태 콜백 등록/notification |
...
<a id="binary-manager-boot-flow"></a>
## Binary 관리자 부팅 흐름
...
- 파티션 유형 `kernel` -> `binary_manager_register_kpart()`
- 파티션 유형 `bin` + 이름 `common`/`app1` -> `binary_manager_register_upart()`
- 파티션 유형 `bootparam` -> `binary_manager_register_bppart()`
- 리소스 파티션 -> `binary_manager_register_respart()`
...
<a id="bootparam-and-ab-partitions"></a>
## Bootparam 및 A/B 파티션
...
<a id="debugging-and-modification-points"></a>
## 디버깅 및 수정 포인트
