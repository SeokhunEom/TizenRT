...
| 설정 | 값 | 의미 |
|---|---:|---|
| `CONFIG_ARCH_ARM` | `y` | ARM 아키텍처 |
| `CONFIG_ARCH_CHIP_AMEBASMART` | `y` | rtl8730e 칩 제품군 |
...
| 세마포 대기/post/try/timed/reset/destroy | `os/kernel/semaphore/sem_wait.c`, `sem_post.c`, `sem_trywait.c`, `sem_timedwait.c`, `sem_tickwait.c`, `sem_waitirq.c`, `sem_reset.c`, `sem_destroy.c` |
| semaphore holder와 priority inheritance | `os/kernel/semaphore/sem_holder.c`, `sem_setprotocol.c`, `sem_recover.c`, `sem_initialize.c` |
| pthread 뮤텍스 공개 API/type | `os/include/pthread.h`, `os/include/sys/types.h` |
...
| 스케줄러 선점 잠금 | `os/kernel/sched/sched_lock.c`, `sched_unlock.c`, `sched_lockcount.c`, `sched_mergepending.c`, `os/kernel/sched/sched.h` |
| 중요 섹션 공개 API | `os/include/tinyara/irq.h` |
| critical section 구현 | `os/kernel/irq/irq_csection.c` |
| 원시 ARM IRQ save/restore | `os/arch/arm/include/armv7-a/irq.h`, `os/arch/arm/include/types.h` |
| 스핀록 퍼블릭 API | `os/include/tinyara/spinlock.h`, `os/arch/arm/include/spinlock.h` |
| spinlock 구현 | `os/kernel/semaphore/spinlock.c`, `os/kernel/irq/irq_spinlock.c`, `os/arch/arm/src/armv7-a/arm_testset.S` |
| rtl8730e 래퍼 | `os/board/rtl8730e/src/component/os_dep/osdep_service_sem.c`, `osdep_service_mutex.c`, `osdep_service_critical.c` |
...
| `rtw_mutex_init()` | `sem_t` 카운트 1, `sem_setprotocol(SEM_PRIO_NONE)` |
| `rtw_mutex_get()` / `rtw_mutex_put()` | `sem_wait()` / `sem_post()` |
| `rtw_spinlock_init()` | `sem_t` 카운트 1, `sem_setprotocol(SEM_PRIO_NONE)` |
